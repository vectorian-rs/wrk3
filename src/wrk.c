...
int main(int argc, char **argv) {
    ...
    if (!strncmp("https", schema, 5)) {
        if ((cfg.s2n_cfg = ssl_init()) == NULL) {
            fprintf(stderr, "unable to initialize TLS: %s\n",
...
void *thread_main(void *arg) {
    ...
    for (uint64_t i = 0; i < thread->connections; i++, c++) {
        ...
        c->thread     = thread;
        if (cfg.s2n_cfg) {
            c->s2n_conn = s2n_connection_new(S2N_CLIENT);
            c->s2n_cfg  = cfg.s2n_cfg;
        } else {
            c->s2n_conn = NULL;
            c->s2n_cfg  = NULL;
        }
